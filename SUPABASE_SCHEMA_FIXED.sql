-- ════════════════════════════════════════════════════════════════════════════════
-- RESPAWN SIGNAL - SUPABASE SCHEMA (FIXED VERSION)
...
CREATE POLICY agency_creators_select_own ON public.agency_creators FOR SELECT USING (auth.uid() = user_id);
CREATE POLICY agency_creators_insert_own ON public.agency_creators FOR INSERT WITH CHECK (auth.uid() = user_id AND EXISTS (SELECT 1 FROM public.agencies WHERE id = agency_id AND user_id = auth.uid()));
CREATE POLICY agency_creators_update_own ON public.agency_creators FOR UPDATE USING (auth.uid() = user_id) WITH CHECK (auth.uid() = user_id AND EXISTS (SELECT 1 FROM public.agencies WHERE id = agency_id AND user_id = auth.uid()));
CREATE POLICY agency_creators_delete_own ON public.agency_creators FOR DELETE USING (auth.uid() = user_id);

-- AGENCY_SCANS Policies
CREATE POLICY agency_scans_select_own ON public.agency_scans FOR SELECT USING (auth.uid() = user_id);
CREATE POLICY agency_scans_insert_own ON public.agency_scans FOR INSERT WITH CHECK (auth.uid() = user_id AND EXISTS (SELECT 1 FROM public.agencies WHERE id = agency_id AND user_id = auth.uid()));
CREATE POLICY agency_scans_update_own ON public.agency_scans FOR UPDATE USING (auth.uid() = user_id) WITH CHECK (auth.uid() = user_id);
...
CREATE POLICY creator_profiles_insert_own ON public.creator_profiles FOR INSERT WITH CHECK (auth.uid() = user_id);
CREATE POLICY creator_profiles_update_own ON public.creator_profiles FOR UPDATE USING (auth.uid() = user_id) WITH CHECK (auth.uid() = user_id);
CREATE POLICY creator_profiles_delete_own ON public.creator_profiles FOR DELETE USING (auth.uid() = user_id);

-- CREATOR_ALERTS Policies
CREATE POLICY creator_alerts_select_own ON public.creator_alerts FOR SELECT USING (auth.uid() = user_id);
CREATE POLICY creator_alerts_insert_own ON public.creator_alerts FOR INSERT WITH CHECK (auth.uid() = user_id);
CREATE POLICY creator_alerts_update_own ON public.creator_alerts FOR UPDATE USING (auth.uid() = user_id) WITH CHECK (auth.uid() = user_id);
CREATE POLICY creator_alerts_delete_own ON public.creator_alerts FOR DELETE USING (auth.uid() = user_id);
...
CREATE TRIGGER update_agency_creators_updated_at BEFORE UPDATE ON public.agency_creators FOR EACH ROW EXECUTE FUNCTION public.update_updated_at_column();
CREATE TRIGGER update_agency_scans_updated_at BEFORE UPDATE ON public.agency_scans FOR EACH ROW EXECUTE FUNCTION public.update_updated_at_column();
...
CREATE TRIGGER update_creator_alerts_updated_at BEFORE UPDATE ON public.creator_alerts FOR EACH ROW EXECUTE FUNCTION public.update_updated_at_column();
CREATE TRIGGER update_analytics_snapshots_updated_at BEFORE UPDATE ON public.analytics_snapshots FOR EACH ROW EXECUTE FUNCTION public.update_updated_at_column();
