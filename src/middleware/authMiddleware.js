...
const PLAN_DEFAULTS = {
  ...
  free: { credits: 50, maxRange: 3, automation: false, unlimitedHistory: false },
  ...
  try {
    ...
    if (!dbUser) {
      const { data: newUser, error: insertErr } = await supabase
        .from('users')
        .insert({ email: user.email, plan: 'free', credits: 50 })
        .select()
        .single();
    ...
    // Attach to request
    req.dbUser = dbUser;
    req.planConfig = PLAN_DEFAULTS[dbUser.plan] || PLAN_DEFAULTS.free;
    next();
  } catch (err) {
